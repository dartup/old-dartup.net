...
class Auth {
  ...
  Auth(this._location, this._server, this._window) {
    _boolStream = onToken.map((s) => s.isNotEmpty)
    ...
    if (!logedIn) {
      ...
      if (_location.currentUri.queryParameters.containsKey('code')) {
        ...
        _server.getToken(_location.currentUri.queryParameters['code']).then((t) => token = t);
      }
    }
  }
  ...
  void login() {
    ...
    _server.getCinetId().then((clinetId) {
      var query = {"client_id": clinetId, "redirect_uri": _location.currentUri.toString(), "scope": "user:email"};
